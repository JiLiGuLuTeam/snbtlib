...
class List(Base, list):
	...
	def __new__(cls, *values):
		if cls.itemtype is None and len(values) > 0:
			cls.itemtype = type(values[0])
		try:
			self = super(List, cls).__new__(cls, [cls.itemtype(i) for i in values])
			self.itemtype = cls.itemtype
			cls.itemtype = None
			return self
		except:
			raise NBTStoreError('初始化 %s 内数据时错误,列表类型为 %s' % (values, cls.itemtype))
